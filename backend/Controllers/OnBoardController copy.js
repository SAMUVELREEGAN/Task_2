    ...
    try {
        if (where === "type") {
            ...
            const [generalDetails] = await db.query(
                `SELECT doctor_name, speciality, clinic_name, address_1, address_2, address_3, address_4, area, pincode_id , user_id 
                 FROM general_details WHERE type_id=?`,
                [type_id]
            );

            // SECURITY DETAILS
            const [securityDetails] = await db.query(
                `SELECT id, mobile_number, email, upload_pan, pan_number, upload_gst, gst_number, is_email, is_mobile, type_id ,user_id
                 FROM security_details WHERE type_id=?`,
                [type_id]
            );
