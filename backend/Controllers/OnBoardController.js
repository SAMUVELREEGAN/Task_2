    ...
    try {
        ...
        if (where === 'details') {
            ...
            if (categoryIdNum === 5) {
                ...
                [license] = license_hospital_id
                    ? await db.query(
                        `SELECT upload_govt_establishment_order, hospital_code,
                         ...
                         WHERE id = ? AND user_id = ?`,
                        [license_hospital_id, user_id]
                    )
                    : [[]];

                [generalDetails] = general_id
                    ? await db.query(
                        `SELECT hospital_name, short_name, address_1, address_2,
                ...
                [securityDetails] = security_detail_id
                    ? await db.query(
            ...
            if (typeIdNum === 1 && [1, 2, 3].includes(categoryIdNum)) {
                ...
                [license] = license_pharmacy_id
                    ? await db.query(
                        `SELECT ${licenseColumnsMap[categoryIdNum]}
                         FROM license_details_pharmacy
                         WHERE id = ? AND user_id = ?`,
                        [license_pharmacy_id, user_id]
                    )
                    : [[]];

                [generalDetails] = general_id
                    ? await db.query(
                        `SELECT pharmacy_name, cathlap, address_1, address_2,
                                address_3, address_4, area, pincode_id
                         FROM general_details
                         WHERE id = ? AND user_id = ?`,
                        [general_id, user_id]
...
exports.postCategories = async (req, res) => {
    ...
    try {
        ...
        let license_id;
        let general_id;
        let security_detail_id;

        /* =====================================================
           🏥 HOSPITAL (category_id = 5)
        ===================================================== */
        if (categoryIdNum === 5) {
            ...
            general_id = generalResult.insertId;
            ...
            security_detail_id = securityResult.insertId;

            // ✅ ONBOARDING UPDATE (HOSPITAL)
            await db.query(
                `UPDATE onboarding_form
                 SET license_hospital_id = ?, 
                     general_id = ?, 
                     security_detail_id = ?
                 WHERE user_id = ? AND type_id = ? AND category_id = ?`,
                [
                    license_id,
                    general_id,
                    security_detail_id,
        ...
        /* =====================================================
           💊 PHARMACY (type_id = 1, category_id = 1|2|3)
        ===================================================== */
        if (typeIdNum === 1 && [1, 2, 3].includes(categoryIdNum)) {
            ...
            const [licenseResult] = await db.query(licenseQuery, licenseValues);
            license_id = licenseResult.insertId;
            ...
            // ✅ ONBOARDING UPDATE (PHARMACY)
            await db.query(
                `UPDATE onboarding_form
                 SET license_pharmacy_id = ?, 
                     general_id = ?, 
                     security_detail_id = ?
                 WHERE user_id = ? AND type_id = ? AND category_id = ?`,
                [
  ...
  if (!user_id) {
            ...
            const [onboarding] = await db.query(
                `SELECT 
                    ...
                    security_detail_id
                 FROM onboarding_form
                 WHERE user_id = ? AND type_id = ? AND category_id =?  AND  sub_category_id =?`,
                [user_id, type_id ,category_id,sub_category_id]
            );
