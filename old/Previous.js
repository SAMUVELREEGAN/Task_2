...
exports.getParmacyType = async(req,res)=>{
       try {
            const [rows] = await db.query(
            "SELECT id,category_name FROM category_name WHERE type_id = 1",
            // [type_id]  
        )
          res.json(rows)
    } catch (err) {
...
exports.getDoctorType = async(req,res)=>{
       try {
            const [rows] = await db.query(
            "SELECT id,category_name FROM category_name WHERE type_id = 2",
            // [type_id]  
        )
          res.json(rows)
    } catch (err) {
...
exports.gethospitalType = async(req,res)=>{
       try {
            const [rows] = await db.query(
            "SELECT id,sub_category_name FROM sub_category WHERE type_id = 2",
            // [type_id]  
        )
          res.json(rows)
    } catch (err) {
...
exports.postHospital = async (req,res)=>{
...
exports.getHospital = async (req,res)=>{
    const { category_id, sub_category_id } = req.params;
    // Hardcode type_id as 2 for hospital since route is /onboard/2/...
    const type_id = 2;
...
exports.getParmacy = async(req,res)=>{
...
exports.postParmacy = async (req,res) =>{
    const { category_id } = req.params;
    const type_id = 1;
    const { license_details, general_details , security_detail } = req.body;
